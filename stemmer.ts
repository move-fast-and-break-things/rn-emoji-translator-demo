// Algorithm from https://popsul.ru/blog/2012/06/post-10.html
...
export default function stemmWord(word: string): string {
  word = word.replace(/ё/gi, 'e');
  const wParts = word.match(DICT.RVRE);
  if (!wParts) {
    return word;
  }
  const start = wParts[1];
  let rv = wParts[2];
  let temp = rv.replace(DICT.PERFECTIVEGROUND_2, '');
  if (temp == rv) {
